  ...
  _createClass(Education, [{
    ...
    value: function setTemplate() {
      return this.$data.map(function (data) {
        return "\n            <li class=\"slide-item item-group\">\n                ".concat(data.image ? "<img src=\"".concat(data.image, "\" alt=\"\">") : "", "\n                ").concat(data.title ? "<span class=\"title\">".concat(data.title, "</span>") : "", "\n                ").concat(data.link ? "<a href=\"".concat(data.link, "\">").concat(data.name, "</a>") : "", "\n                ").concat(data.location ? "<span>".concat(data.location, "</span>") : "", "\n                ").concat(data.department ? "<span class=\"stretch\">".concat(data.department, "</span>") : "", "\n            </li>\n        ");
      }).join("");
    }
  }, {
    key: "render",
...
(0, _utils.getData)("http://localhost:3000/imageSlide", function (data) {
  return new Education(document.querySelector('#education .slider-group'), data.education);
});
