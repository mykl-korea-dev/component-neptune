        ...
        if(Array.isArray(this.$data) || this.$data.data) {
            ...
            return $data.map(data => {
                ...
                <div class="mykl-radio">
                    <input class="radio-input" type="radio" name="${this.name}" id="${this.name}-${data[id]}" value="${data[id]}"  ${data[checked] === trueChecked ? "checked" : ''}>
                    <label class="radio-label" for="${this.name}-${data[id]}">${data[value]}</label>
                </div>
            `
            }).join('');
        } else {
